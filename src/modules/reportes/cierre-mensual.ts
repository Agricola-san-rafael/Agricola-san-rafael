...
export async function generarExcelCierre(mes: string): Promise<Buffer> {
  ...
  const filas: [string, string | number | null][] = [
    ...
    ["Ingresos del transporte (tarifas; sin tarifa = a costo)", cierre.fletes.ingresos],
    ["Resultado del transporte (ingresos - costo real)", cierre.fletes.resultadoTransporte],
